...
const SHARED_DIR_NAME: &str = "hb_shared_files";
...
async fn main() {
    ...
    let shared_dir = Path::new(&args.dir).join(SHARED_DIR_NAME);

    if shared_dir.exists() {
        let has_subdirs = std::fs::read_dir(&shared_dir)
            .ok()
    ...
    let state = AppState {
        ...
        shared_dir: Arc::new(shared_dir.clone()),
    };
    ...
    println!("Shared files directory: {}", shared_dir.to_str().unwrap());
    println!("Server running on http://{}:{}", local_ip, args.port);
...
async fn upload_handler(
    ...
    while let Ok(Some(mut field)) = multipart.next_field().await {
        if let Some(display_name) = field.file_name() {
            ...
            let filepath = state.shared_dir.join(&stored_name);
            ...
            if file_history.len() > state.file_history_limit {
                if let Some(removed) = file_history.pop_back() {
                    let removed_path = state.shared_dir.join(&removed.stored_name);
